...
def findAllAlbums(limit: int = 5000):
    albums = PHOTOS.list_albums(limit=limit)["data"]["list"]
    albumNames = [album["name"] for album in albums]
    return albums, albumNames

def default_onFind(folder: dict[str], albums: list[str]):
    print(f'Found folder {folder["id"]} at {folder["name"]}')

def search_teams_folders(root_id, albums: list[str], name: str, case_sensitive: bool = False, onFind = default_onFind) -> "tuple[list[int], list[str]]":
    queue: list[int] = []
...
def onFind(folder: dict[str], albums: list[str]):
    print(f'Found folder {folder["id"]} at {folder["name"]}')
    
    create_album(folder, albums)


def create_album(folder: dict[str], albumNames: list[str]):
...
def delete_albums(albums: list[dict[str]]):
    for album in albums:
